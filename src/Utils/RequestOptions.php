...
class RequestOptions
{
     ...
     * @param bool $strict when true, forbid string form and arbitrary keys in array form
     *
     * @throws \Paystack\Exception\InvalidArgumentException
     *
     * @return RequestOptions
     */
    public static function parse($options, $strict = false)
    {
        ...
        if (\is_string($options)) {
            if ($strict) {
                $message = 'Do not pass a string for request options. If you want to set the '
                    . 'API key, pass an array like ["secret" => <secret>] instead.';
        ...
        if (\is_array($options)) {
            ...
            if (\array_key_exists('paystack_account', $options)) {
                $headers['Paystack-Account'] = $options['paystack_account'];
                unset($options['paystack_account']);
            }
            if (\array_key_exists('paystack_version', $options)) {
                $headers['Paystack-Version'] = $options['paystack_version'];
                unset($options['paystack_version']);
            }
        ...
        $message = 'The second argument to Paystack API method calls is an '
           . 'optional per-request apiKey, which must be a string, or '
           . 'per-request options, which must be an array. (HINT: you can set '
           . 'a global apiKey by "Paystack::setApiKey(<apiKey>)")';
