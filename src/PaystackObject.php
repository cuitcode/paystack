 ...
 */
class PaystackObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    /** @var RequestOptions */
    protected $_opts;
    ...
    {
        list($id, $this->_retrieveOptions) = Util::normalizeId($id);
    ...
    public function __set($k, $v)
    {
        if (static::getPermanentAttributes()->includes($k)) {
            throw InvalidArgument::badArgument(
        ...
        $this->_values[$k] = Util::convertToPaystackObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "Paystack Notice: Undefined property of {$class} instance: {$k}. "
                    . "HINT: The {$k} attribute was set in the past, however. "
                    . 'It was then wiped when refreshing the object '
                    . "with the result returned by Paystack's API, "
                    . 'probably as a result of a save(). The attributes currently '
                    . "available on this object are: {$attrs}";
            Paystack::getLogger()->error($message);

            return $nullval;
        }
        $class = \get_class($this);
        Paystack::getLogger()->error("Paystack Notice: Undefined property of {$class} instance: {$k}");
    ...
    /**
     * This unfortunately needs to be public to be used in Util.
     *
     * @param array $values
     * @param null|array|string|RequestOptions $opts
     *
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof PaystackObject) {
        ...
        if ($partial) {
            ...
            $removed = new Set(\array_diff(\array_keys($this->_values), \array_keys($values)));
        }
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a PaystackObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = PaystackObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util::convertToPaystackObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a PaystackObject. A PaystackObject may contain modified
            //      values within in that its parent PaystackObject doesn't know about.
            //
            $original = \array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof PaystackObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (Util::isList($value)) {
                ...
                return Util::convertToPaystackObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof PaystackObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Paystack object.
     *
     * @return string the JSON representation of the Paystack object
     */
    ...
    /**
     * Sets all keys within the PaystackObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any PaystackObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        ...
        if ($obj instanceof PaystackObject) {
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof PaystackObject) {
            $values = $obj->_values;
        } else {
            throw InvalidArgument::badArgument(
    ...
    /**
     * @return null|ApiResponse The last response from the Paystack API
     */
